...
try:
    ...
    for fileName in files:
        ...
        if os.path.isfile(filePath) and utility.ImageUtils.isPictureExtensionFile(filePath):
            epd.Init_4Gray()
            Himage = Image.open(filePath)
            Himage = utility.ImageUtils.autoRotateImage(Himage) # take care of orientation using exif
    
            # Process to greyscale (4 color) and increase constrast
            GImage = Himage.convert('L', dither=Image.FLOYDSTEINBERG)
            GImage.quantize(colors=4)
            # Increase image constrast
            GImage = ImageOps.autocontrast(GImage)
            # Resize and crop image to fit aspect_ratio of epd screen
            GImage = utility.ImageUtils.resizeImage(GImage, epd.width, epd.height)
